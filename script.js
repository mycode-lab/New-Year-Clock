document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateTime() {
    let now = correctedTime
      ? new Date(correctedTime.getTime() + (Date.now() - lastServerCheck))
      : new Date();
    ...
    newYearDayEl.innerHTML = `<i class="fas fa-calendar-day"></i> New Year Day: ${newYear.toLocaleDateString(
      undefined,
      { weekday: "long" }
    )}`;
    ...
    const daysLeft = Math.floor(
      (timeLeft % (1000 * 60 * 60 * 24 * 30.44)) / (1000 * 60 * 60 * 24)
    );
  ...
  async function checkTimeWithServer() {
    try {
      ...
      const dateDifference = Math.abs(
        (serverDate - localDate) / (1000 * 60 * 60 * 24)
      );
  ...
  function handleOfflineStatus() {
    ...
    if (correctedTime) {
      const serverTime = new Date(
        correctedTime.getTime() + (Date.now() - lastServerCheck)
      );
      const timeDifference = Math.abs((serverTime - localTime) / 1000);
      const serverDate = new Date(serverTime.toDateString());
      const localDate = new Date(localTime.toDateString());
